...
async function main() {
  ...
  const relacionesPath = path.join(
    __dirname,
    "../src/data/relaciones_data.json"
  );
  ...
  try {
    ...
    const speciesInput = (fakeData.species || []).map((e: any) => ({
      id: e.id,
      name: e.name,
    }));
    ...
    const breedsInput = (fakeData.breeds || []).map((r: any) => ({
      id: r.id,
      name: r.name,
    ...
    for (let i = breedsInput.length; i < 10; i++) {
      ...
      breedsInput.push({
        id: randomUUID(),
        name: `Raza ${i + 1}`,
        speciesId: speciesInput[speciesIdx].id,
      });
    }
    ...
    const rolesInput = (fakeData.roles || []).map((r: any) => ({
      id: r.id,
      name: r.name,
    ...
    const regionsInput = (fakeData.regions || []).map((rg: any) => ({
      ...
      activationDate: rg.activationDate
        ? new Date(rg.activationDate)
        : undefined,
      latitude: rg.latitude != null ? toNum(rg.latitude) : undefined,
      longitude: rg.longitude != null ? toNum(rg.longitude) : undefined,
      coverageRadiusKm:
        rg.coverageRadiusKm != null ? toNum(rg.coverageRadiusKm) : undefined,
    }));
    ...
    const userRolesInput = (fakeData.userRoles || []).map((ur: any) => ({
      id: ur.id,
      userId: ur.userId,
      roleId: ur.roleId,
    }));
    for (let i = userRolesInput.length; i < 10; i++) {
      userRolesInput.push({
        ...
        roleId: rolesInput[i % rolesInput.length].id,
      });
    }
    ...
    const petsInput = (mascotasData.pets || []).map((m: any, idx: number) => {
      ...
      const lat =
        m.lostLocationLat != null
          ? Number(m.lostLocationLat)
          : jitter(city.lat, 0.05);
      const lon =
        m.lostLocationLon != null
          ? Number(m.lostLocationLon)
          : jitter(city.lon, 0.05);
      ...
      return {
        ...
        rewardAmount:
          m.rewardAmount != null ? toNum(m.rewardAmount) : undefined,
    ...
    for (let i = adoptionsInput.length; i < 10 && i < petsInput.length; i++) {
      adoptionsInput.push({
        id: randomUUID(),
        petId: petsInput[i].id,
        requirements: `Req auto ${i + 1}`,
        contactInfo: `contacto${i + 1}@demo.com`,
      });
    }
    ...
    const petImagesInput = (relacionesData.petImages || []).map(
      (img: any, idx: number) => ({
        id: img.id,
        petId: img.petId,
        url: img.url || "",
    ...
    for (let i = petImagesInput.length; i < 10; i++) {
      ...
      petImagesInput.push({
        id: randomUUID(),
        petId: pet.id,
        url: `https://picsum.photos/seed/pet-extra-${i}/900/700`,
        isPrimary: i % 3 === 0,
      });
    }
    ...
    const walkersInput = (relacionesData.walkers || []).map(
      (w: any, idx: number) => ({
        id: w.id,
        userId: w.userId,
    ...
    const expansionsInput = (relacionesData.expansions || []).map(
      (ex: any) => ({
        id: ex.id,
        regionId: ex.regionId,
        expansionDate: new Date(ex.expansionDate),
      })
    );
    for (let i = expansionsInput.length; i < 10; i++) {
      expansionsInput.push({
        id: randomUUID(),
        regionId: regionsInput[i % regionsInput.length].id,
        expansionDate: new Date("2024-02-01T00:00:00Z"),
      });
    }
